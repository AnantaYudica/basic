#ifndef BASIC_ERR_ID_RECORD_H_
#define BASIC_ERR_ID_RECORD_H_
...
public:
    ...
    constexpr Record(const FlagType & flag, 
        const ErrorCodeValueType& err_code_val) noexcept;
    constexpr Record(const FlagType & flag,
        const ErrorSystemCategoryValueType& err_sys_categ_val, 
        const ErrorSystemCodeValueType& err_sys_code_val) noexcept;
...
constexpr Record<TErrorCodeValue, TErrorSystemCategoryValue,
    TErrorSystemCodeValue>::Record(const FlagType & flag,
        ...
        const ErrorSystemCodeValueType& err_sys_code_val) noexcept :
            m_flag(flag),
            m_categ(err_sys_categ_val),
            m_code(err_sys_code_val)
{}
            ...
            m_code(std::move(mov.m_code))
{
    mov.m_categ = Record<TErrorCodeValue, TErrorSystemCategoryValue,
        TErrorSystemCodeValue>{basic::err::id::Flag{}}.m_categ;
    mov.m_code = Record<TErrorCodeValue, TErrorSystemCategoryValue,
        TErrorSystemCodeValue>{basic::err::id::Flag{}}.m_code;
}
